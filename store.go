...
// methods of our store
// if something goes wrong on either of them, an error is returned
type Store interface {
	CreateNote(note *Note) error
	GetNotes() ([]*Note, error)
}

// 'dbStore' struct will implement the 'Store' interface
// it takes in the sql DB connection object, which represents the db connection
type dbStore struct {
	db *sql.DB
}
